...
<div class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-lg bg-white/80 backdrop-blur-lg rounded-2xl shadow-2xl p-8 space-y-6">
        <div class="text-center">
            ...
            <p class="mt-2 text-gray-600">Please sign in to your {{ $title }} account</p>
        </div>

        <form class="space-y-6" id="{{ $formId }}">
            <div class="space-y-4">
                @if ($formItems)
                    @foreach ($formItems as $item)
                        <div>
                            <label for="{{ $item['id'] }}"
                                class="block text-sm font-medium text-gray-700 mb-1">{{ $item['label'] }}</label>
                            <input id="{{ $item['id'] }}" name="{{ $item['id'] }}" type="{{ $item['id'] }}"
                                required
